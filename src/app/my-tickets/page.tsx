...
import { Ticket, ArrowLeft, Calendar, MapPin, User, Printer, Sparkles, Clock3, CheckCircle2, AlertCircle, ImageIcon } from 'lucide-react';
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-[#7B5A58] font-mono">
              View your registrations, payment verification status, and entry ticket passes.
            </p>
          </div>
        </div>
        ...
        {!currentUser ? (
          <div className="bg-[#FAF0EE] border-3 border-[#5A3B38] rounded-[32px] p-12 text-center max-w-md mx-auto retro-shadow">
            ...
            <p className="text-xs text-[#7B5A58] mt-2 mb-6">
              Log in to track your payment approval status and access your entry ticket stubs.
            </p>
            <button
              onClick={() => setAuthModalOpen(true)}
              className="bg-[#5A3B38] text-[#FAF0EE] border-2 border-[#5A3B38] px-8 py-3.5 rounded-full font-bold text-xs retro-shadow hover:scale-105 transition-all"
            >
                ...
                >
                  <div>
                    <div className="flex items-center justify-between pb-3 border-b-2 border-dashed border-[#5A3B38] mb-4">
                      ...
                      {isApproved ? (
                        <span className="text-[10px] font-mono font-bold uppercase text-[#059669] flex items-center gap-1">
                          <CheckCircle2 className="w-3.5 h-3.5" /> Confirmed
                        </span>
                      ) : (
                        <span className="text-[10px] font-mono font-bold uppercase text-[#D97706] flex items-center gap-1">
                    ...
                    <div className="space-y-1.5 text-xs text-[#342224] mb-4">
                      ...
                      <div className="flex items-center gap-2 text-[#7B5A58]">
                        <User className="w-4 h-4 text-[#5A3B38]" /> Seats: {reg.guest_count} spot(s)
                      </div>
                    </div>
                    ...
                    {reg.payment_screenshot && (
                      <div className="bg-[#F2D8D5] border border-[#5A3B38] p-2.5 rounded-2xl mb-4 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-[11px] text-[#342224] font-bold">
                          <ImageIcon className="w-4 h-4 text-[#5A3B38]" /> Payment Receipt
                        </div>
                        <button
                  ...
                  {isApproved ? (
                    <button
                      ...
                      className="w-full bg-[#D7B4A8] border-2 border-[#5A3B38] hover:bg-[#5A3B38] hover:text-[#FAF0EE] text-[#342224] py-2.5 rounded-full font-bold text-xs flex items-center justify-center gap-2 transition-all mt-2"
                    >
                    ...
                    <div className="bg-[#FEF3C7] border-2 border-[#D97706] text-[#92400E] p-3 rounded-2xl text-center text-xs font-bold mt-2">
                      ⏳ Payment screenshot is under review by organizers. Ticket pass will unlock here once approved!
                    </div>
                  )}
                </div>
              );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="relative bg-[#FAF0EE] border-3 border-[#5A3B38] rounded-3xl p-4 max-w-lg w-full">
            <button
              ...
              className="absolute top-2 right-2 bg-[#F2D8D5] border border-[#5A3B38] p-1.5 rounded-full"
            >
              ✕
            </button>
